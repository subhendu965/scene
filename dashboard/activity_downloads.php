...
<!------------------------------------------------------------------------------------------->
  <div id="head_bar_5239">
    <div id="like_logo_5239" class="jkml36985">&nbsp;</div>
		...
		<?php
		$query="SELECT user_id,total_download FROM hb_scene_users WHERE user_id='".$_SESSION['hb_user']['user_id']."' LIMIT 1";
		if($rows=mysqli_query($hb_scene_db,$query)){
			if($row=mysqli_fetch_array($rows, MYSQLI_BOTH)){
				if($row['user_id']==$_SESSION['hb_user']['user_id']){
					$total_download_745=$row['total_download'];
				}else{
...
if(($total_download_745)<=0){
?>
	<div>
		You did not view anything till now.
	</div>
<?php
		}else{
?>
		<div id="header_right_side_5239" class="jkml36985">
			<div id="header_right_side_upper_5239">You have viewed exactly</div>
			<div id="header_right_side_lower_5239" >
				<div id="like_number_5239" class="kmg4125"><?php echo $total_download_745; ?></div>
				<div id="header_right_side_lower_t_5239" class="kmg4125"> images</div>
			</div>
		</div>
<?php
		}
?>
  ...
  <div id="showing_area_5239">
    ...
    <?php ///read from xml files and show///
		$download_xml_url=$rootpath."scene/user_data/user_".$_SESSION['hb_user']['username']."/download_history.xml";
		if(!file_exists($download_xml_url)){
			die("You did not download any image till now.");
		}

		$xml=simplexml_load_file($download_xml_url);

...
<!------------------------------------------------------------------------------------------->
  <div id="head_bar_5239">
    <div id="like_logo_5239" class="jkml36985">&nbsp;</div>
		...
		<?php
		$query="SELECT user_id,total_view FROM hb_scene_users WHERE user_id='".$_SESSION['hb_user']['user_id']."' LIMIT 1";
		if($rows=mysqli_query($hb_scene_db,$query)){
			if($row=mysqli_fetch_array($rows, MYSQLI_BOTH)){
				if($row['user_id']==$_SESSION['hb_user']['user_id']){
					$total_view_745=$row['total_view'];
				}else{
...
?>
		<div id="header_right_side_5239" class="jkml36985">
			...
			<div id="header_right_side_lower_5239" >
				<div id="like_number_5239" class="kmg4125"><?php echo $total_view_745; ?></div>
...
?>
  ...
  <div id="showing_area_5239">
    ...
    <?php ///read from xml files and show///
		$view_xml_url=$rootpath."scene/user_data/user_".$_SESSION['hb_user']['username']."/view_history.xml";
		if(!file_exists($view_xml_url)){
			die("You did not view any image till now.");
		}

		$xml=simplexml_load_file($view_xml_url);

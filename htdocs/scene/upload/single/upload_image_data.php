...
if(isset($_SESSION['hb_scene']['single_file_upload_user_96523']) && $_SESSION['hb_scene']['single_file_upload_user_96523']=="Gfx62Pliu26E"){
	...
	#ll77_main_div #ll77_img_data_info li{
		...
		padding-left:20px;
		display:block;
		background:url('/images/ok_right.png') no-repeat;
...
/////////////////////update name
		...
		if($rows=mysqli_query($hb_scene_db,$query)){
			if($row=mysqli_fetch_array($rows,MYSQLI_BOTH)){
				if($_POST['imageid']==$row['uniq_image_id']){
					$url=$row['url'];
					$xml=simplexml_load_file($rootpath."scene/user_gallery/user_".$_SESSION['hb_user']['username']."/".$url.".xml");
					$tmpxml=new SimpleXMLElement($xml->asXML());
					$tmpxml->story=$_POST['story'];
					$tmpxml->asXML($rootpath."scene/user_gallery/user_".$_SESSION['hb_user']['username']."/".$url.".xml");
	...
	echo "<div id='ll77_img_upload_link'>
		<a id='ll77_img_upload_single' href='http://scene.hazrabrothers.com/upload/single/'><div>Upload single image</div></a>
		<a id='ll77_img_upload_album' href='http://scene.hazrabrothers.com/upload/albums/'><div>Upload an album</div></a>
	</div>

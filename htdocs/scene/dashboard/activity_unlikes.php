...
<!------------------------------------------------------------------------------------------->
  <div id="head_bar_5239">
    <div id="like_logo_5239" class="jkml36985">&nbsp;</div>
		...
		<?php
		$query="SELECT user_id,total_unliked FROM hb_scene_users WHERE user_id='".$_SESSION['hb_user']['user_id']."' LIMIT 1";
		if($rows=mysqli_query($hb_scene_db,$query)){
			if($row=mysqli_fetch_array($rows, MYSQLI_BOTH)){
				if($row['user_id']==$_SESSION['hb_user']['user_id']){
					$total_unlikes_745=$row['total_unliked'];
				}else{
		...
		if(file_exists($comment_xml_url)){
			...
			foreach($tmpxml201->children() as $comment){
				...
				foreach ($likes558->children() as $like) {
					if($like->current_reaction=="unlike"){
						$unlike_comment_69=$unlike_comment_69+1;
					}
					//echo "push";
				}
			}
...
if($unlike_comment_69+$total_unlikes_745<=0){
?>
	<div>
		You have not unlike anything till now.
	</div>
<?php
		}else{
?>
		<div id="header_right_side_5239" class="jkml36985">
			<div id="header_right_side_upper_5239">You have unliked exactly</div>
			<div id="header_right_side_lower_5239" >
				<div id="like_number_5239" class="kmg4125"><?php echo ($total_unlikes_745+$unlike_comment_69); ?></div>
				<div id="header_right_side_lower_t_5239" class="kmg4125">times (<?php echo $total_unlikes_745; ?> on images and <?php echo $unlike_comment_69; ?> on comments)</div>
			</div>
		</div>
<?php
		}
?>
    ...
    <?php ///read from xml files and show///
		...
		if(!file_exists($like_xml_url)){
			die("You did not unlike any image till now.");
		}

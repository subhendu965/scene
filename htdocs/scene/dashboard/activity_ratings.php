...
<!------------------------------------------------------------------------------------------->
  <div id="head_bar_5239">
    <div id="like_logo_5239" class="jkml36985">&nbsp;</div>
		...
		<?php
		$query="SELECT user_id,total_rating FROM hb_scene_users WHERE user_id='".$_SESSION['hb_user']['user_id']."' LIMIT 1";
		if($rows=mysqli_query($hb_scene_db,$query)){
			if($row=mysqli_fetch_array($rows, MYSQLI_BOTH)){
				if($row['user_id']==$_SESSION['hb_user']['user_id']){
					$total_rating_745=$row['total_rating'];
				}else{
...
if(($total_rating_745)<=0){
?>
	<div>
		You did not rate anything till now.
	</div>
<?php
		}else{
?>
		<div id="header_right_side_5239" class="jkml36985">
			<div id="header_right_side_upper_5239">You rated exactly</div>
			<div id="header_right_side_lower_5239" >
				<div id="like_number_5239" class="kmg4125"><?php echo $total_rating_745; ?></div>
				<div id="header_right_side_lower_t_5239" class="kmg4125">times</div>
			</div>
		</div>
<?php
		}
?>
  ...
  <div id="tab_menu_bar_5239">
    <div class="mnh521" hb_code="1"><div>5*</div></div>
    <div class="mnh521" hb_code="2"><div>4*</div></div>
	<div class="mnh521" hb_code="3"><div>3*</div></div>
    <div class="mnh521" hb_code="4"><div>2*</div></div>
	<div class="mnh521" hb_code="5"><div>1*</div></div>
  ...
  <div id="showing_area_5239">
    ...
    <?php ///read from xml files and show///
		$rating_xml_url=$rootpath."scene/user_data/user_".$_SESSION['hb_user']['username']."/rating_list.xml";
		if(!file_exists($rating_xml_url)){
			die("You did not rate any image till now.");
		}

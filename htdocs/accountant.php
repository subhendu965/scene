...
if(isset($_GET['senderid']) && isset($_GET['sendersecurity'])){
	if(decryptData($_GET['sendersecurity'])=="Anj5213964xmj0d"){
					...
					if($rows=mysqli_query($hb_db,$query)){
						if($row=mysqli_fetch_array($rows,MYSQLI_BOTH)){
							if($user_id==$row['user_id']){
								...
								$nr=encryptData($nr);
								$sendersecurity="Anj5213964xgh0d";
								$sendersecurity=encryptData($sendersecurity);
								$url="location:http://accounts.hazrabrothers.com/accountant.php?senderid=accountpage&";
	...
	}elseif(decryptData($_GET['sendersecurity'])=="Anj5213973xmj0d"){
		...
		$sendersecurity="Anj5213973xgh0d";
		$sendersecurity=encryptData($sendersecurity);
		$url="location:http://accounts.hazrabrothers.com/accountant.php?senderid=accountpage&";
		$url.="sendersecurity=".$sendersecurity;
		///header($url);

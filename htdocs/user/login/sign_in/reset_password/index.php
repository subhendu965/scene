...
//////////////////////////////////
if(isset($_SESSION['hb_user']['reset_password']) && isset($_SESSION['hb_user']['reset_password_email']) && ($_SESSION['hb_user']['reset_password']=="fgp8595")){


}else{
	die("Sorry Some problem inside. Try again later or contact with our team.");
}
///////////////////////////////////
?>
<!doctype html>
...
if(isset($_POST['submit'])){
	...
	$email=$_SESSION['hb_user']['reset_password_email'];
	if(isset($_POST['new_password']) && !empty($_POST['new_password']) && isset($_POST['confirm_password']) && !empty($_POST['confirm_password'])){
		...
		$query="SELECT email,username FROM hb_user_main_security WHERE email='".mysqli_real_escape_string($hb_db,$_SESSION['hb_user']['reset_password_email'])."' LIMIT 1";
		if($rows=mysqli_query($hb_db,$query)){
			if($row=mysqli_fetch_array($rows,MYSQLI_BOTH)){
				if($row['email']==$_SESSION['hb_user']['reset_password_email']){
    	...
    	$query="UPDATE hb_user_main_security SET password='".mysqli_real_escape_string($hb_db,$_POST['new_password'])."' WHERE email='".mysqli_real_escape_string($hb_db,$_SESSION['hb_user']['reset_password_email'])."' LIMIT 1";
			  ...
			  if(mysqli_query($hb_db,$query)){
					...
					if(file_exists($user_data_path."user_password_history.xml")){
						...
						///////////////////change in database/////////////////
						$query="UPDATE hb_user_forget_password SET rsq='VFD',rcode='VFD' WHERE email='".mysqli_real_escape_string($hb_db,$_SESSION['hb_user']['reset_password_email'])."' LIMIT 1";
						if(mysqli_query($hb_db,$query)){
						...
						unset($_SESSION['hb_user']['reset_password']);
						unset($_SESSION['hb_user']['reset_password_email']);
...
<div id="main">
	...
	<form name="reset_password_form" action="index.php" method="post" onsubmit="return verifyData();">
		...
		<div id="input_element">
			<label>New Password:
				<input type="password" name="new_password" autofocus onblur="validatePassword();" />
			</label>
		</div>
